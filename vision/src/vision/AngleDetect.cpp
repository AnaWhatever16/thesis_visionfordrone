...
namespace aerox{
    ...
    float AngleDetect::scoreSelection(std::vector<Match> &_scores){
        ...
        // for (size_t i = 0; i < _scores.size(); i++){ //we look for lines that are repeated
        //     for (size_t j = 1; j < _scores.size()-i; j++){
        //         if (_scores[i].flag_eval == 1) break; // if this line was already found repeated we don't continue
                
        //         if (_scores[i].id_line == _scores[i+j].id_line){ //same line found
        //             if (_scores[i].score > _scores[i+j].score){ //we eliminate higher value
        //                 _scores[i].flag_eval = 1; 
        //             }
        //             else if (_scores[i].score <_scores[i+j].score){ //we eliminate higher value
        //                 _scores[i+j].score = 1;
        //             }
        //             else{ //if they have same value, doesn't mean they are the minimum. We use the first one.
        //                 _scores[i+j].flag_eval = 1;
        //             }
        //         }
        //     }
        // }
